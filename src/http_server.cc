...
int http_onConnectionCompleted(struct tcp_connection *tcpConnection) {
    aurora_msgx("connection completed");
...
int process_status_line(char *start, char *end, struct http_request *httpRequest) {
    int size = end - start;
    //method
    char *space = static_cast<char *>(memmem(start, end - start, " ", 1));
    assert(space != NULL);
    int method_size = space - start;
    httpRequest->method = static_cast<char *>(malloc(method_size + 1));
    strncpy(httpRequest->method, start, space - start);
    httpRequest->method[method_size + 1] = '\0';

    //url
    start = space + 1;
    space = static_cast<char *>(memmem(start, end - start, " ", 1));
    assert(space != NULL);
    int url_size = space - start;
    httpRequest->url = static_cast<char *>(malloc(url_size + 1));
    strncpy(httpRequest->url, start, space - start);
    httpRequest->url[url_size + 1] = '\0';

    //version
    start = space + 1;
    httpRequest->version = static_cast<char *>(malloc(end - start + 1));
    ...
    while (httpRequest->current_state != REQUEST_DONE) {
        if (httpRequest->current_state == REQUEST_STATUS) {
            ...
            if (crlf) {
                ...
                char *colon = static_cast<char *>(memmem(start, request_line_size, ": ", 2));
                if (colon != NULL) {
                    char *key = static_cast<char *>(malloc(colon - start + 1));
                    strncpy(key, start, colon - start);
                    key[colon - start] = '\0';
                    char *value = static_cast<char *>(malloc(crlf - colon - 2 + 1));
...
int http_onMessage(struct buffer *input, struct tcp_connection *tcpConnection) {
    aurora_msgx("get message from tcp connection %s", tcpConnection->name);
...
int http_onWriteCompleted(struct tcp_connection *tcpConnection) {
    aurora_msgx("write completed");
    return 0;
}

//连接关闭之后的callback
int http_onConnectionClosed(struct tcp_connection *tcpConnection) {
    aurora_msgx("connection closed");
    if (tcpConnection->request != NULL) {
        http_request_clear(static_cast<http_request*>(tcpConnection->request));
...
struct http_server *http_server_new(struct event_loop *eventLoop, int port,
                                    ...
                                    int threadNum) {
    struct http_server *httpServer = new http_server;
