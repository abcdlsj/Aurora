...
struct thread_pool *thread_pool_new(struct event_loop *mainLoop, int threadNumber) {
    ...
    struct thread_pool *threadPool = new thread_pool;
...
void thread_pool_start(struct thread_pool *threadPool) {
    ...
    threadPool->eventLoopThreads = static_cast<event_loop_thread *>(malloc(
            threadPool->thread_number * sizeof(struct event_loop_thread)));
