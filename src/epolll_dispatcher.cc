...
void *epoll_init(struct event_loop *eventLoop) {
    epoll_dispatcher_data *epollDispatcherData = static_cast<epoll_dispatcher_data *>(malloc(
            sizeof(epoll_dispatcher_data)));
    ...
    epollDispatcherData->events = static_cast<epoll_event *>(calloc(MAXEVENTS, sizeof(struct epoll_event)));
...
int epoll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) {
    ...
    aurora_msgx("epoll_wait wakeup, %s", eventLoop->thread_name);
    for (i = 0; i < n; i++) {
        ...
        if (epollDispatcherData->events[i].events & EPOLLIN) {
            aurora_msgx("get message channel fd==%d for read, %s", epollDispatcherData->events[i].data.fd,
        ...
        if (epollDispatcherData->events[i].events & EPOLLOUT) {
            aurora_msgx("get message channel fd==%d for write, %s", epollDispatcherData->events[i].data.fd,
                        eventLoop->thread_name);
