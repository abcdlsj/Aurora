...
struct http_response *http_response_new() {
    struct http_response *httpResponse = new http_response;
    httpResponse->body = NULL;
    httpResponse->statusCode = Unknown;
    httpResponse->statusMessage = NULL;
    httpResponse->response_headers = static_cast<response_header *>(malloc(
            sizeof(struct response_header) * INIT_RESPONSE_HEADER_SIZE));
