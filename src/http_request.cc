...
struct http_request *http_request_new() {
    struct http_request *httpRequest = new http_request;
    httpRequest->method = NULL;
    httpRequest->current_state = REQUEST_STATUS;
    httpRequest->version = NULL;
    httpRequest->url = NULL;
    httpRequest->request_headers = static_cast<request_header *>(malloc(
            sizeof(struct http_request) * INIT_REQUEST_HEADER_SIZE));
