...
struct buffer *buffer_new() {
    struct buffer *buffer1 = new buffer;
    if (!buffer1)
        return NULL;

    buffer1->data = static_cast<char *>(malloc(INIT_BUFFER_SIZE));
...
void make_room(struct buffer *buffer, int size) {
    ...
    if (buffer_front_spare_size(buffer) + buffer_writeable_size(buffer) >= size) {
        ...
        buffer->data = static_cast<char *>(tmp);
...
char *buffer_find_CRLF(struct buffer *buffer) {
    char *crlf = static_cast<char *>(memmem(buffer->data + buffer->readIndex, buffer_readable_size(buffer), CRLF, 2));
    return crlf;
}