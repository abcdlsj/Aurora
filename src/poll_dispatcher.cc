...
void *poll_init(struct event_loop *eventLoop) {
    struct poll_dispatcher_data *pollDispatcherData = new poll_dispatcher_data;

    //初始化pollfd数组，这个数组的第一个元素是listen_fd，其余的用来记录将要连接的connect_fd
    pollDispatcherData->event_set = static_cast<pollfd *>(malloc(sizeof(struct pollfd) * INIT_POLL_SIZE));
    // 用-1表示这个数组位置还没有被占用
    int i;
...
int poll_add(struct event_loop *eventLoop, struct channel *channel1) {
    ...
    aurora_msgx("poll added channel fd==%d, %s", fd, eventLoop->thread_name);
    if (i >= INIT_POLL_SIZE) {
...
int poll_del(struct event_loop *eventLoop, struct channel *channel1) {
    ...
    aurora_msgx("poll delete channel fd==%d, %s", fd, eventLoop->thread_name);
    if (i >= INIT_POLL_SIZE) {
...
int poll_update(struct event_loop *eventLoop, struct channel *channel1) {
    ...
    aurora_msgx("poll updated channel fd==%d, %s", fd, eventLoop->thread_name);
    if (i >= INIT_POLL_SIZE) {
...
int poll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) {
    ...
    for (i = 0; i < INIT_POLL_SIZE; i++) {
        ...
        //有事件发生
        if (pollfd.revents > 0) {
            aurora_msgx("get message channel i==%d, fd==%d, %s", i, socket_fd, eventLoop->thread_name);
