...
int event_loop_handle_pending_add(struct event_loop *eventLoop, int fd, struct channel *channel) {
    aurora_msgx("add channel fd == %d, %s", fd, eventLoop->thread_name);
    ...
    if ((map)->entries[fd] == NULL) {
        ...
        //add channel
        struct event_dispatcher *eventDispatcher = const_cast<event_dispatcher *>(eventLoop->eventDispatcher);
...
int event_loop_handle_pending_remove(struct event_loop *eventLoop, int fd, struct channel *channel1) {
    ...
    struct channel *channel2 = static_cast<channel *>(map->entries[fd]);
...
int event_loop_handle_pending_update(struct event_loop *eventLoop, int fd, struct channel *channel) {
    aurora_msgx("update channel fd == %d, %s", fd, eventLoop->thread_name);
    struct channel_map *map = eventLoop->channelMap;
...
int channel_event_activate(struct event_loop *eventLoop, int fd, int revents) {
    struct channel_map *map = eventLoop->channelMap;
    aurora_msgx("activate channel fd == %d, revents=%d, %s", fd, revents, eventLoop->thread_name);
    ...
    struct channel *channel = static_cast<struct channel *>(map->entries[fd]);
...
int handleWakeup(void *data) {
    ...
    aurora_msgx("wakeup, %s", eventLoop->thread_name);
}
...
struct event_loop *event_loop_init_with_name(char *thread_name) {
    struct event_loop *eventLoop = new event_loop;
    ...
    eventLoop->quit = 0;
    eventLoop->channelMap = new channel_map;
...
#else
    aurora_msgx("set poll as dispatcher, %s", eventLoop->thread_name);
...
int event_loop_run(struct event_loop *eventLoop) {
    ...
    struct event_dispatcher *dispatcher = const_cast<event_dispatcher *>(eventLoop->eventDispatcher);

    if (eventLoop->owner_thread_id != pthread_self()) {
        exit(1);
    }

    aurora_msgx("event loop run, %s", eventLoop->thread_name);
    ...
    aurora_msgx("event loop end, %s", eventLoop->thread_name);
    return 0;
}
