...
int handle_write(void *data) {
    ...
    if (nwrited > 0) {
        ...
        aurora_msgx("handle_write for tcp connection %s", tcpConnection->name);
    }
...
tcp_connection_new(int connected_fd, struct event_loop *eventLoop,
    ...
    struct tcp_connection *tcpConnection = new tcp_connection;
...
int tcp_connection_send_data(struct tcp_connection *tcpConnection, void *data, int size) {
    ...
    if (!fault && nleft > 0) {
        ...
        buffer_append(output_buffer, static_cast<char *>(data) + nwrited, nleft);
...
void tcp_connection_shutdown(struct tcp_connection *tcpConnection) {
    if (shutdown(tcpConnection->channel->fd, SHUT_WR) < 0) {
        aurora_msgx("tcp_connection_shutdown failed, socket == %d", tcpConnection->channel->fd);
    }
}